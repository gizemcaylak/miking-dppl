...
recursive
let cluster = lam q. lam trees. lam partitions. lam maxAge. lam seqLen. lam n. lam d.
  if eqi n 1 then trees else
  let seqLenF = int2float seqLen in
  let k = 4.0 in
  let beta = divf k seqLenF in
  ...
  cluster q new_trees newPartitions age seqLen (subi n 1) dNew
end

let model = lam trees. lam seqLength. lam q. lam partitions. lam distanceMatrix.
  cluster q trees partitions 0.0 seqLength (length trees) distanceMatrix
