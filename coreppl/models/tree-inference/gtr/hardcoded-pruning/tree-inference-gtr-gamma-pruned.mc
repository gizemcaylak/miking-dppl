...
let cluster = lam q. lam trees. lam maxAge. lam seqLen. lam n. lam pi. lam rs.
  ...
  let node_msg = mapIndex (lam i.
    ...
    let childMsgs = zipWithIndex (lam j. lam child. lam qtc.
      let values = map (lam i.
        let ps = map (lam qt. ctmc i qt) qtc in
        let marg = foldl (lam acc. lam p. addSeq acc p) (head ps) (tail ps) in
        map (lam e. mulf e 0.25) marg) [0,1,2,3] in
      let msg = get msg j in
      let in_msg = map (lam p. let t = foldl2 (lam acc. lam pi. lam lci. addf acc (mulf pi lci)) 0. p msg in t) values in
      let log_likes = getLogLikes msg pi in
...
let model = lam.
  ...
  let q = gtr pi er in
  let alpha = (assume (Exponential 1.0)) in
  let srs = create seqLength (lam. discretizedGammaSupport alpha (divf 1. alpha) 4) in
