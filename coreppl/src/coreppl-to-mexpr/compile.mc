...
lang CompileModels = ReplaceHigherOrderConstants + DPPLDelayedSampling + DPPLPruning + PhaseStats + LoadRuntime + DPPLDelayedReplace + DPPLTransformCancel + MExprANFAll + DPPLExtract + InferenceInterface
    ...
    mapMapWithKey
      (lam id. lam model.
        ...
        match mapLookup method runtimes with Some entry then
          ...
          endPhaseStats log "remove-model-definitions-one" ast;
          ast
        else
  ...
  | {extractAst = extractAst, params = modelParams} ->
    ...
    endPhaseStats log "compile-inference-one" ast;
    ...
    endPhaseStats log "insert-model-params-one" ast;
    ...
    endPhaseStats log "replace-ty-dist-one" ast;
...
-- TODO(vipa, 2024-12-04): model transform for noInfer is missing
lang CPPLLoader
  = MCoreLoader
...
lang CorePPLFileTypeLoader = CPPLLoader + GeneratePprintLoader + MExprGeneratePprint
  ...
  sem _loadDPPLFileHelper : all x. Map String x -> String -> (Set String, [Decl], Expr)
  sem _loadDPPLFileHelper includedFiles = | path ->
    let args =
    ...
    recursive let f = lam newFiles. lam decls. lam ast.
      match exprAsDecl ast with Some (decl, ast) then
        match infoDecl decl with Info {filename = filename} then
          if mapMem filename includedFiles
          then f newFiles decls ast
          else f (setInsert filename newFiles) (snoc decls decl) ast
        else
          warnSingle [infoVal path 0 0 0 0] "Decl with NoInfo from file";
          printLn (pprintDeclCode 2 pprintEnvEmpty decl).1;
          f newFiles decls ast
      else (newFiles, decls, ast) in
    match f (setEmpty cmpString) [] ast with (newFiles, decls, expr) in
    let newFiles = if setMem path newFiles
      then newFiles
      else setInsert path newFiles in
    (newFiles, decls, expr)
  ...
  sem _loadFile path = | (FCorePPL {isModel = isModel}, loader & Loader x) ->
    ...
    match _loadDPPLFileHelper x.includedFiles path with (newFiles, decls, expr) in
    let newFiles = mapMap (lam. _symEnvEmpty) newFiles in
    let hasInfer =
    ...
    let loader =
      if or (not isModel) hasInfer then
        foldl _addDeclExn (Loader {x with includedFiles = mapUnion x.includedFiles newFiles}) decls
      else
        ...
        let loader = foldl _addDeclExn
          (Loader {x with includedFiles = mapUnion x.includedFiles newFiles})
          beforeFile in
...
lang MExprCompile =
  ...
  --   endPhaseStats log "load-runtimes" ast;
  ...
  --   endPhaseStats log "combine-runtimes" ast;
  ...
  --   endPhaseStats log "symbolize" corepplAst;
  --   let corepplAst = typeCheck corepplAst in
  --   endPhaseStats log "type-check" corepplAst;
  ...
  --     end
  --   in
  --   endPhaseStats log "ode-transform" corepplAst;
  ...
  --     else corepplAst
  --   in
  --   endPhaseStats log "lift-dual-nums" corepplAst;
  ...
  --     (corepplAst, models)
  --   in
  --   endPhaseStats log "extract-infer" corepplAst;
  ...
  --   endPhaseStats log "compile-models" corepplAst;
  ...
  --   endPhaseStats log "replace-delay-types" corepplAst;
  --   let corepplAst = transformDistributions corepplAst in
  --   endPhaseStats log "transform-distributions" corepplAst;
  ...
  --   endPhaseStats log "add-top-names" corepplAst;
  ...
  --   endPhaseStats log "replace-dppl-keywords" corepplAst;
  ...
  --   in
  --   endPhaseStats log "eliminate-duplicate-externals" mainAst;
  ...
  --   endPhaseStats log "insert-models" prog;
  ...
  --     else prog
  --   in
  --   endPhaseStats log "provide-dual-number-impl" prog;
  ...
  --   in
  --   endPhaseStats log "transform-elementary" prog;
  --   let prog = eliminateDuplicateExternals (bind_ elementaryRuntime prog) in
  --   endPhaseStats log "eliminate-duplicate-externals-2" prog;
