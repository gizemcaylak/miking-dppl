...
lang TransformDsDist = TransformDist + MExprPPL +DPPLParser
 ...
 | _ -> env
  ...
  | DPoisson {lambda = lambda} ->
    let lambda = match assignDCons env lambda with Some x then x else (conapp_ "DelayedGraph_FloatParam" lambda) in
...
lang DPPLDelayedTransform = TransformDsDist
  ...
  | TmVar a -> -- equivalent to value(X)
    ...
    else-/ match varType with Some (DelayParam _) then
      appf2_ (var_ "value") sampleT (conapp_ "DelayedGraph_DelayParam" (TmVar a))
    else (TmVar a)
  ...
  | TmVar t ->
    match mapLookup t.ident env.delayedEnv with Some v then
    v else TmVar t
  | TmLet ({body=TmDelayed p} &t) ->
...
/-
let a = delay (Gaussian 0. 1.) in
let b = Gaussian a 1 in
lam x:DelayInt. 
-/
