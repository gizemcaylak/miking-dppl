...
type Digraph v = { adj : Map v [v], parents : Map v [v] }
...
let digraphEmpty : all v. (v -> v -> Int) -> Digraph v =
  lam cmpv.  {adj = mapEmpty cmpv, parents = mapEmpty cmpv}
...
let digraphEdgesTo : all v. v -> Digraph v -> [DigraphEdge v] =
  lam v. lam g.
  map (lam t : v. (t, v))
    (mapLookupOrElse (lam. error "Lookup failed") v g.parents)
...
let digraphSuccessors : all v. v -> Digraph v -> [v] = lam v. lam g.
...
let digraphPredeccessors : all v. v -> Digraph v -> [v] = lam v. lam g.
  ...
  let predecessors = map (lam t : DigraphEdge v. t.0) incoming in
  setToSeq (setOfSeq (digraphCmpv g) predecessors)
...
let digraphAddVertexCheck : all v. v -> Digraph v -> Digraph v =
  lam v. lam g.
  {adj = mapInsert v [] g.adj,parents = mapInsert v [] g.parents}
...
let digraphAddUpdateVertex : all v. v -> Digraph v -> Digraph v =
  ...
  let parentsList = mapLookupOrElse (lam. error "Lookup failed") v g.parents in
  let m2 = mapRemove v g.parents in
  let m2 = foldl (lam acc. lam v2.
          let parentsLst = mapLookupOrElse (lam. error "Lookup failed") v2 acc in
          let newParentsLst = map (lam e:v. if digraphEqv g e v then v else e) parentsLst in
          mapInsert v2 newParentsLst acc) m2 (mapKeys m2) in
  {adj=mapInsert v edgeList m,parents = mapInsert v parentsList m2}
...
let digraphAddEdge : all v. v -> v -> Digraph v -> Digraph v =
  ...
  in
  {adj = mapInsert v1 (snoc oldEdgeList v2) g.adj,parents = mapInsert v2 (cons v1 oldParentsList) g.parents}
...
let digraphRemoveEdge : all v. v -> v -> Digraph v -> Digraph v =
  ...
  let newIncoming = filter (lam o : v. (not ((digraphEqv g) from o))) incoming in
  {adj = mapInsert from newOutgoing g.adj, parents = mapInsert to newIncoming g.parents}
...
let g = digraphAddVertices [RandomVarV {ident=4,state=0},RandomVarV {ident=5,state=1},RandomVarV {ident=2,state=0}] g in
let g = digraphAddEdges [(RandomVarV {ident=4,state=0},RandomVarV {ident=5,state=1}),(RandomVarV {ident=3,state=0},RandomVarV {ident=2,state=1})] g in

let g = digraphAddUpdateVertex (RandomVarV{ident=3,state=4}) g in
utest digraphSuccessors (RandomVarV{ident=0,state=1}) g with [(RandomVarV{ident=1,state=1})] in
let g = digraphAddUpdateVertex (RandomVarV{ident=0,state=4}) g in
utest digraphPredeccessors (RandomVarV{ident=1,state=1}) g with [(RandomVarV{ident=0,state=1})] in
utest digraphPredeccessors (RandomVarV{ident=2,state=0}) g with [(RandomVarV{ident=3,state=4})] in
utest digraphVertices g with [RandomVarV{ident=0,state=4},RandomVarV {ident=1,state=1},RandomVarV {ident=2,state=0},RandomVarV {ident=3,state=4},RandomVarV {ident=4,state=0},RandomVarV {ident=5,state=1}] in
()
